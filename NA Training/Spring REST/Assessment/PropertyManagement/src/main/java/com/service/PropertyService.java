...
public class PropertyService {
	...
	public PropertyService() {
		...
		PropertyInfo propertyObj2 = new PropertyInfo(10025, "Not Approved", "30 x 50", "Bangalore", 9500000,
				"Available");
	...
	public PropertyInfo viewPropertyById(Integer propertyId) {
		return propertyList.stream()
				.filter(prop -> prop.getPropertyId() == propertyId)
				.findAny().get();
	}

	public ArrayList<PropertyInfo> viewPropertyByLocationAndCostRange(String location, double lowLimit,
			double highLimit) {
		return new ArrayList<>(propertyList.stream().filter(prop -> prop.getLocation().equals(location))
	...
	public boolean deleteProperty(Integer propertyId) {
		PropertyInfo propertyInfo = viewPropertyById(propertyId);
		return propertyList.remove(propertyInfo);
	}
